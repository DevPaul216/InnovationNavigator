...
COLOR_PARTLY_COMPLETED = "rgb(180, 200, 255)"  # New color for partly completed
...
def init_flow_graph(connection_states, completed_templates, blocked_templates, partly_completed_templates=None):
    if sst.update_graph:
        nodes = []
        for i, template_name in enumerate(sst.template_config.keys()):
            ...
            if template_name == "Start":
                ...
                elif partly_completed_templates and template_name in partly_completed_templates:
...
def init_graph():
    connection_states = {}
    completed_templates = []
    partly_completed_templates = []  # New list for partly completed
    for template_name, template_config in sst.template_config.items():
        is_fulfilled = True
        ...
        if is_required:
            elements = template_config["elements"]
            required_elements = [e for e in elements if sst.elements_config[e]["required"]]
    ...
    return connection_states, completed_templates, blocked_templates, partly_completed_templates
